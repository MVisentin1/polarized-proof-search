...
Ltac T_rfc path ::=
  ...
  | [|- rfc _ ?b] => let b' := (eval hnf in b) in
    ...
    | Or _ _ => (apply rfc_R_Or_1 ; [> T_exh | T_rfc path]) || (apply rfc_R_Or_2 ; [> T_exh | T_rfc path])
...
Ltac  T_lfc path ::=
  ...
  | [|- lfc _ ?b _] => let b' := (eval hnf in b) in
    ...
    | Atom Neg _ => apply lfc_I_l ; [>  T_exh | apply Neg_atom | apply Is_atom ]

    (* Left focus on negative conjunction. Try proving B1, if it fails, try proving B2*)
    | AndN _ _ => (apply lfc_L_AndN_1 ; [>  T_exh | T_lfc path ]) || (apply lfc_L_AndN_2 ; [> T_exh | T_lfc path ])
...
Ltac T_ufc_decide_right path ::=
  ...
  | [|- ufc_b ?c ?k] =>
    ...
    (T_is_positive k' ; apply ufc_b_R_f ; [> T_exh | T_positive | T_rfc path]) || T_ufc_decide_left path
  end
.
...
Ltac T_ufc_decide_left_private context path :=
  ...
  lazymatch context' with
    ...
    (T_is_negative b' ; T_noloop context' b path; eapply (@ufc_b_L_f _ b' _) ; [> T_exh | T_has_entry | T_negative | T_lfc npath]) || T_ufc_decide_left_private rest path
