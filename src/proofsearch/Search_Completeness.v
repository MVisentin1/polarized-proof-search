...
From LJF Require Import SharedLogic Predicates Sequents Pndctx Search_Procedure LJFPS_Rules LJFh_Rules Schemes Subsequent_Preservation ProofTerms.
From Equations Require Import Equations.

Definition returns_some_proof {A B : Type} (r : option (A + B)) : Prop :=
  match r with Some (inl _) => True | _ => False end.

Definition returns_some_disproof {A B : Type} (r : option (A + B)) : Prop :=
  match r with Some (inr _) => True | _ => False end.

Definition returns_none {A B : Type} (r : option (A + B)) : Prop :=
  match r with None => True | _ => False end.

Lemma returns_some_proof_inv [A B: Type] {r : option (A + B)} :
  returns_some_proof r -> exists p : A, r = Some (inl p).
Proof.
...
Lemma try_Lf_complete :
    forall (C : pndctx) (K : o) 
        (try_N : forall N : o, In N (pndctx_list C) -> option ({p : pterm | verify p (Slfc C N K)} + (~ sequent_derivable (Slfc C N K))))
            (neg_ctx : list o)
...
Proof.
    ...
    - destruct H ; subst.
        + specialize (H0 (proj1 (Forall_inv H_Forall))). simp try_Lf.
            apply returns_some_proof_inv in H0. destruct H0 as [[x v] H]. rewrite H. simpl. apply I.
        + specialize (IHneg_ctx (Forall_inv_tail H_Forall) M H H0).
            ...
            destruct s.
            -- destruct s. simpl. apply I.
            -- destruct x. simpl. rewrite H1. simpl. apply I.
            -- destruct x. simpl. rewrite H1. simpl. apply I.
Qed.
...
Proof.
    ...
    - destruct D ; simp search ; cbv zeta.
        + destruct p ; simp search ; cbv zeta.
            -- specialize (H init (subp_bct_boxR (Bracketable_pos (Atom Pos n) (Pos_atom n)) Hsub) Hstack_NoDup Hstack_sub).
                apply returns_some_proof_inv in H. destruct H as [[x v] H].
    ...
    - simp search.
        ...
        apply returns_some_proof_inv in H. destruct H as [[x v] H]. rewrite H. simpl. 
        specialize (H0 init (proj2 (subp_bct_AndNR Hsub)) Hstack_NoDup Hstack_sub).
        apply returns_some_proof_inv in H0. destruct H0 as [[x0 v0] H0]. cbn in H0. cbn. rewrite H0. simpl. apply I.
    - simp search.
    ...
    - rewrite search_equation_9. destruct (InA_dec (eqA:=pndctx_o_eq) pndctx_o_eq_dec (C, K) Hs).
        ...
        + simpl. 
            ...
            -- apply (try_Lf_wrapper_complete C K _ N i n0). intros.
                ...
                apply returns_some_proof_inv in H. destruct H as [[x v] H]. rewrite H. simpl. apply I.
            -- destruct (Decidability.positive_dec K).
                ++ simp search. destruct (search init (Srfc C K) (subp_ept_Rf Hsub) ((C, K) :: Hs) (NoDupA_cons (x:=(C, K)) n1 Hstack_NoDup) (Forall_cons (C, K) Hsub Hstack_sub)).
                    --- destruct s.
                        +++ destruct s. simpl. apply I.
                        +++ simpl. apply returns_some_proof_inv in H0. destruct H0 as [[x0 v0] H0]. cbn in H0. cbn. rewrite H0. simpl. apply I.
                    --- simpl. apply returns_some_proof_inv in H0. destruct H0 as [[x0 v0] H0]. cbn in H0. cbn. rewrite H0. simpl. apply I.
                ++ simp search. apply returns_some_proof_inv in H0. destruct H0 as [[x0 v0] H0]. cbn in H0. cbn. rewrite H0. simpl. apply I.
    - rewrite search_equation_9. destruct (InA_dec (eqA:=pndctx_o_eq) pndctx_o_eq_dec (C, P) Hs).
        ...
        + simpl. destruct (Decidability.positive_dec P).
            -- simpl. specialize (H init (subp_ept_Rf Hsub) (NoDupA_cons (x:=(C, P)) n0 Hstack_NoDup) (Forall_cons (C, P) Hsub Hstack_sub)).
                apply returns_some_proof_inv in H. destruct H as [[x v] H]. cbn in H. cbn. rewrite H. simpl. apply I.
            -- contradiction.
    ...
    - inversion p ; subst.
        + rewrite search_equation_18 ; cbv zeta.
            specialize (H init (subp_lfc_Rl Hsub) Hstack_NoDup Hstack_sub). apply returns_some_proof_inv in H. destruct H as [[x v] H]. rewrite H. simpl. apply I.
    ...
    - rewrite search_equation_23.
        specialize (H init (subp_lfc_AndNL_2 Hsub) Hstack_NoDup Hstack_sub). apply returns_some_proof_inv in H. destruct H as [[x v] H].
    ...
    - inversion n ; subst.
        + rewrite search_equation_27 ; cbv zeta.
            specialize (H init (subp_rfc_Rr Hsub) Hstack_NoDup Hstack_sub). apply returns_some_proof_inv in H. destruct H as [[x v] H]. rewrite H. simpl. apply I.
