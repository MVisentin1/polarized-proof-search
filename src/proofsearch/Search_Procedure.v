...
From LJF Require Import SharedLogic Decidability
    ...
    Termination_Measures LJFPS_Inversion Subsequent_Preservation Focus_Decision_Set ProofTerms ProofTerms_Soundness.
From Equations Require Import Equations.

Equations try_Lf {C : pndctx} {K : o}
    (try_N : forall N, In N (pndctx_list C) -> option ({p : pterm | verify p (Slfc C N K)} + ~ sequent_derivable (Slfc C N K)))
    (neg_ctx : list o)
    (H_Forall : Forall (fun N => In N (pndctx_list C) /\ negative N) neg_ctx)
    : option ({p : pterm | verify p (Sept C nil K)} + Forall (fun N => ~ sequent_derivable (Slfc C N K)) neg_ctx) :=

    try_Lf try_N nil _ := Some (inr (Forall_nil (fun N => ~ sequent_derivable (Slfc C N K)))) ;

    try_Lf try_N (N :: neg_rest) H_Forall 
        with try_N N (proj1 (Forall_inv H_Forall)) := {
            | Some (inl (exist _ p V))  =>
                let H1 := verify_soundness V in
                let Hin := proj1 (Forall_inv H_Forall) in
                let Hbr := LJFPS_bracketable_goal_lfc H1 in
                let Hne := proj2 (Forall_inv H_Forall) in
                Some (inl (exist _ (pept_Lf N p) (vept_Lf Hin Hbr Hne V))) ;
            | Some (inr H1)             with try_Lf try_N neg_rest (Forall_inv_tail H_Forall) := {
                | Some (inl (exist _ p V))  => Some (inl (exist _ p V)) ;
                | Some (inr H2)             => Some (inr (Forall_cons N H1 H2)) ;
                | None                      => None
                }
...
Definition try_Lf_wrapper {C : pndctx} {K : o}
    (try_N : forall N, In N (pndctx_list C) -> option ({p : pterm | verify p (Slfc C N K)} + ~ sequent_derivable (Slfc C N K)))
    : option ({p : pterm | verify p (Sept C nil K)} + Forall (fun N => ~ sequent_derivable (Slfc C N K)) (filter negative_b (pndctx_list C))) :=
...
Equations search 
    ...
    : option ({p : pterm | verify p seq} + ~ sequent_derivable seq)
    by wf
    ...
    search init (Sept C nil K) Hsub stack Hstack_NoDup Hstack_sub with InA_dec pndctx_o_eq_dec (C, K) stack := {
        ...
        | right Hin     with positive_dec K := {
            | left Hpk  with search init (Srfc C K) (subp_ept_Rf Hsub) ((C, K) :: stack) (NoDupA_cons Hin Hstack_NoDup) (Forall_cons (C, K) Hsub Hstack_sub) := {
                | Some (inl (exist _ p1 V1))    => Some (inl (exist _ (pept_Rf p1) (vept_Rf Hpk V1)))
                | Some (inr H1)     with try_Lf_wrapper 
                    (fun N (HinN : In N (pndctx_list C)) => search init (Slfc C N K) (subp_ept_Lf HinN Hsub) ((C, K) :: stack) (NoDupA_cons Hin Hstack_NoDup) (Forall_cons (C, K) Hsub Hstack_sub)) := {
                        | Some (inl (exist _ p2 V2))    => Some (inl (exist _ p2 V2))
                        | Some (inr H2)                 => Some (inr (ept_nil_disproof_pos Hpk H1 H2))
                        | None                          => None
                    }
            ...
            | right Hpk     with try_Lf_wrapper
                (fun N (HinN : In N (pndctx_list C)) => search init (Slfc C N K) (subp_ept_Lf HinN Hsub) ((C, K) :: stack) (NoDupA_cons Hin Hstack_NoDup) (Forall_cons (C, K) Hsub Hstack_sub)) := {
                    | Some (inl (exist _ p V))  => Some (inl (exist _ p V))
                    | Some (inr H1)             => Some (inr (ept_nil_disproof_neg Hpk H1))
                    | None                      => None
            }
        }
    } ;
    ...
    search init (Sept C ((Atom Pos n) :: L) K) Hsub stack Hstack_NoDup Hstack_sub :=
        ...
        | Some (inl (exist _ p V))  => Some (inl (exist _ (pept_boxL p) (vept_boxL (LJFPS_bracketable_goal_ept (verify_soundness V)) Hpe V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (ept_boxL_inv Hpe H)))
        | None                      => None
        end ;
    ...
    with search init (Sept C (B2 :: B1 :: L) K) (subp_ept_AndPL Hsub) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p V))  => Some (inl (exist _ (pept_AndPL p) (vept_AndPL (LJFPS_bracketable_goal_ept (verify_soundness V)) V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (ept_AndPL_inv H)))
        | None                      => None
    } ;
    ...
    with search init (Sept C (B1 :: L) K) (proj1 (subp_ept_OrL Hsub)) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p1 V1))  with search init (Sept C (B2 :: L) K) (proj2 (subp_ept_OrL Hsub)) stack Hstack_NoDup Hstack_sub := {
            | Some (inl (exist _ p2 V2))    => Some (inl (exist _ (pept_OrL p1 p2) (vept_OrL (LJFPS_bracketable_goal_ept (verify_soundness V2)) V1 V2)))
            | Some (inr H2)                 => Some (inr (fun H => H2 (proj2 (ept_OrL_inv H)))) ;
            | None                          => None
            }
        | Some (inr H1)             => Some (inr (fun H => H1 (proj1 (ept_OrL_inv H)))) ;
    ...
    with search init (Sept C L K) (subp_ept_TrueL Hsub) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p V))  => Some (inl (exist _ (pept_TrueL p) (vept_TrueL (LJFPS_bracketable_goal_ept (verify_soundness V)) V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (ept_TrueL_inv H)))
        | None                      => None
    } ;

    search init (Sept C (FF :: L) K) Hsub stack Hstack_NoDup Hstack_sub
    with bracketable_dec K := {
        | left Hbr      => Some (inl (exist _ pept_FalseL (vept_FalseL Hbr))) 
    ...
    search init (Sbct C L (Atom Pos n)) Hsub stack Hstack_NoDup Hstack_sub := 
        ...
        | Some (inl (exist _ p V))  => Some (inl (exist _ (pbct_boxR p) (vbct_boxR Hbr V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (bct_boxR_inv Hbr H)))
        | None                      => None
        end ;
    ...
    with search init (Sbct C L B1) (proj1 (subp_bct_AndNR Hsub)) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p1 V1))    with search init (Sbct C L B2) (proj2 (subp_bct_AndNR Hsub)) stack Hstack_NoDup Hstack_sub := {
            | Some (inl (exist _ p2 V2))    => Some (inl (exist _ (pbct_AndNR p1 p2) (vbct_AndNR V1 V2)))
            | Some (inr H2)                 => Some (inr (fun H => H2 (proj2 (bct_AndNR_inv H))))
            | None                          => None
            }
        | Some (inr H1)     => Some (inr (fun H => H1 (proj1 (bct_AndNR_inv H))))
    ...
    with search init (Sbct C (B1 :: L) B2) (subp_bct_ImpR Hsub) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p V))  => Some (inl (exist _ (pbct_ImpR p) (vbct_ImpR V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (bct_ImpR_inv H)))
        | None                      => None
    } ;
    ...
    search init (Slfc C (Atom Pos n) K) Hsub stack Hstack_NoDup Hstack_sub :=
        ...
        | Some (inl (exist _ p V))  => Some (inl (exist _ (plfc_Rl p) (vlfc_Rl (LJFPS_bracketable_goal_ept (verify_soundness V)) Hpos V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (lfc_Rl_inv Hpos H)))
        | None                      => None
        end ;
    ...
    with o_eq_dec (Atom Neg n) K := {
        | left Heq  => Some (inl (exist _ plfc_Il
            (match Heq in (_ = K) return verify plfc_Il (Slfc C (Atom Neg n) K) with
                | eq_refl => vlfc_Il (Neg_atom n) (Is_atom Neg n)
                end))) ;
        | right Heq => Some (inr (fun H => Heq (lfc_Il_NK_eq (Is_atom Neg n) (Neg_atom n) H)))
    } ;
    ...
    with search init (Slfc C B1 K) (subp_lfc_AndNL_1 Hsub) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p1 V1))    => Some (inl (exist _ (plfc_AndNL_1 p1) (vlfc_AndNL_1 (LJFPS_bracketable_goal_lfc (verify_soundness V1)) V1)))
        | Some (inr H1)                 with search init (Slfc C B2 K) (subp_lfc_AndNL_2 Hsub) stack Hstack_NoDup Hstack_sub := {
            | Some (inl (exist _ p2 V2))    => Some (inl (exist _ (plfc_AndNL_2 p2) (vlfc_AndNL_2 (LJFPS_bracketable_goal_lfc (verify_soundness V2)) V2)))
            | Some (inr H2)                 => Some (inr (fun H => match lfc_AndNL_inv H with
                | or_introl H3      => H1 H3
                | or_intror H3      => H2 H3
                end ))
            | None                      => None
    ...
    with search init (Srfc C B1) (proj1 (subp_lfc_ImpL Hsub)) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p1 V1))    with search init (Slfc C B2 K) (proj2 (subp_lfc_ImpL Hsub)) stack Hstack_NoDup Hstack_sub := {
            | Some (inl (exist _ p2 V2))    => Some (inl (exist _ (plfc_ImpL p1 p2) (vlfc_ImpL (LJFPS_bracketable_goal_lfc (verify_soundness V2)) V1 V2)))
            | Some (inr H2)                 => Some (inr (fun H => H2 (proj2 (lfc_ImpL_inv H))))
            | None                          => None
            }
        | Some (inr H1)                 => Some (inr (fun H => H1 (proj1 (lfc_ImpL_inv H))))
    ...
    search init (Srfc C (Atom Neg n)) Hsub stack Hstack_NoDup Hstack_sub :=
        ...
        | Some (inl (exist _ p V))  => Some (inl (exist _ (prfc_Rr p) (vrfc_Rr Hneg V)))
        | Some (inr H1)             => Some (inr (fun H => H1 (rfc_Rr_inv Hneg H)))
        | None                      => None
        end ; 
    ...
    with in_dec o_eq_dec (Atom Pos n) (pndctx_list C) := {
        | left Hin  => Some (inl (exist _ prfc_Ir (vrfc_Ir Hin (Pos_atom n) (Is_atom Pos n))))
        | right Hin => Some (inr (fun H => Hin (rfc_Ir_inv (Pos_atom n) (Is_atom Pos n) H)))
    } ;
    ...
    with search init (Srfc C B1) (proj1 (subp_rfc_AndPR Hsub)) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p1 V1))    with search init (Srfc C B2) (proj2 (subp_rfc_AndPR Hsub)) stack Hstack_NoDup Hstack_sub := {
            | Some (inl (exist _ p2 V2))    => Some (inl (exist _ (prfc_AndPR p1 p2) (vrfc_AndPR V1 V2)))
            | Some (inr H2)                 => Some (inr (fun H => H2 (proj2 (rfc_AndPR_inv H))))
            | None                          => None
            }
        | Some (inr H1)                 => Some (inr (fun H => H1 (proj1 (rfc_AndPR_inv H))))
    ...
    with search init (Srfc C B1) (subp_rfc_OrR_1 Hsub) stack Hstack_NoDup Hstack_sub := {
        | Some (inl (exist _ p1 V1))    => Some (inl (exist _ (prfc_OrR_1 p1) (vrfc_OrR_1 V1)))
        | Some (inr H1)                 with search init (Srfc C B2) (subp_rfc_OrR_2 Hsub) stack Hstack_NoDup Hstack_sub := {
            | Some (inl (exist _ p2 V2))    => Some (inl (exist _ (prfc_OrR_2 p2) (vrfc_OrR_2 V2)))
            | Some (inr H2)                 => Some (inr (fun H => match rfc_OrR_inv H with
                | or_introl H3  => H1 H3
                | or_intror H3  => H2 H3
                end ))
            | None              => None
    ...
    search init (Srfc C TT) Hsub stack Hstack_NoDup Hstack_sub := Some (inl (exist _ prfc_TrueR vrfc_TrueR)) ;

    search init (Srfc C FF) Hsub stack Hstack_NoDup Hstack_sub := Some (inr (rfc_FF_unprovable)).
